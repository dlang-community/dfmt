...
private:
    ...
    void formatAt()
    {
        ...
        if (currentIs(tok!"("))
        {
            ...
            if (index < tokens.length && tokens[index - 1].line < tokens[index].line)
                ...
                || isBasicType(tokens[index].type) || currentIs(tok!"identifier")))
            write(" ");
    }
    ...
    void formatLeftBrace()
    {
        ...
        else if (astInformation.funLitStartLocations.canFindIndex(tokens[index].index))
        {
            ...
            if (peekBackIs(tok!")"))
                write(" ");
            auto e = expressionEndIndex(index);
            immutable bool hasComment = tokens[index .. e].canFind!((a, b) => a.type == b)(
                tok!"comment");
            immutable int l = currentLineLength + tokens[index .. e].map!(a => tokenLength(a)).sum();
            writeToken();
    ...
    void newline()
    {
        ...
        if (hasCurrent)
        {
            ...
            {
                while (indents.topIsTemp() && (peekBackIsOneOf(true, tok!"}",
                        tok!";") && indents.top != tok!";"))
                    indents.pop();
                indentLevel = indents.indentLevel;
            }
...
const pure @safe @nogc:
    ...
    bool isBlockHeader(int i = 0) nothrow
    {
        ...
        return t == tok!"for" || t == tok!"foreach"
            ...
            || t == tok!"out" || t == tok!"catch" || t == tok!"with"
